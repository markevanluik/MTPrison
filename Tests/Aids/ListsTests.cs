...
namespace MTPrison.Tests.Aids {
    [TestClass] public class ListsTests : IsTypeTested {
        ...
        [TestMethod] public void RemoveTest() {
            ...
            Assert.AreEqual(0, empty.Remove(x => x.Equals("1")));  // trying remove "1", 0 removes

            var list = new List<dynamic> { 1, 1, 3};
            Assert.AreEqual(2, list.Remove(x => x.Equals(1)));     // trying remove 2  1's
        ...
        [TestMethod] public void ContainsItemTest() {
            var list = new List<dynamic>();
            Assert.IsFalse(list.ContainsItem(x => x.Equals(1)));

            list.Add(2); list.Add(3); list.Add("5");
            Assert.IsTrue(list.ContainsItem(x => x.Equals("5")));   // Equals() is safer with lambda
        }
    }
}
