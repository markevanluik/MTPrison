...
namespace MTPrison.Tests.MTPrisonApp {
    [TestClass] public class CellsPageTests : BasePageTests {
        [TestMethod] public async Task CellsIndexPageTest() => await GetIndexPageTest<ICellsRepo, Cell, CellData>("Cells", x => new Cell(x));
        [TestMethod] public async Task CellsEditPageTest() {
            (CellData d, string html) = await GetEditPageTest<ICellsRepo, Cell, CellData>("Cells", x => new Cell(x));
            isNotNull(d);
            isTrue(d, html);
        }
        [TestMethod] public async Task CellsDetailsPageTest() {
            (CellData d, string html) = await GetDetailsPageTest<ICellsRepo, Cell, CellData>("Cells", x => new Cell(x));
            isNotNull(d);
            isTrue(d, html);
        }
        [TestMethod] public async Task CellsDeletePageTest() {
            (CellData d, string html) = await GetDeletePageTest<ICellsRepo, Cell, CellData>("Cells", x => new Cell(x));
            isNotNull(d);
            isTrue(d, html);
        }
        [TestMethod] public async Task CellsCreatePageTest() {
            (CellData d, string html) = await GetCreatePageTest<ICellsRepo, Cell, CellData>("Cells", x => new Cell(x));
            isNotNull(d);
        }
